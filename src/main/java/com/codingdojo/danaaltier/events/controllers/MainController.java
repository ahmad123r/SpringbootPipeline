...
public class MainController {
	...
	public String viewEvent(@PathVariable Long id, @Valid @ModelAttribute("messageObj") Message message, BindingResult result, Model model, HttpSession session) {
		if(session.getAttribute("userId") == null) {
			return "redirect:/";
		}
		User user = eventsService.findUserById((Long) session.getAttribute("userId"));
	...
	public String editPage(@PathVariable Long id, @ModelAttribute Event event, Model model, HttpSession session) {
		if(session.getAttribute("userId") == null) {
			return "redirect:/";
		}
		User user = eventsService.findUserById((Long)session.getAttribute("userId"));
	...
	public String editEvent(@Valid @PathVariable Long id, @ModelAttribute Event event, BindingResult result, Model model, HttpSession session) {
		User user = eventsService.findUserById((Long)session.getAttribute("userId"));
		if(eventsService.findEventById(id).getUser().getId() == user.getId()) {
			if(result.hasErrors()) {
				model.addAttribute("event", eventsService.findEventById(id));
